...
const upload_Avatar = (req, res) => {
  const { id } = req.params;
  const url = `${VARABLES.domain}${req.file.path}`;

  const query = `UPDATE auth_user SET avatar=? WHERE id=?`;
  db.query(query, [url, id], (error, result) => {
    if (error) return res.status(404).send(error.message);
    res.status(201).send({ message: "Mandate Uploaded" });
  });
...
const update_Staff = (req, res) => {
  ...
  db.query(
    ...
    (error, result) => {
      ...
      res
        .status(201)
        .send({ message: `${first_name} ${last_name} Account Updated` });
    }
  );
};
...
const handle__Pwd_Change = (req, res) => {
  const { id, username } = req.user;
  const query = `UPDATE auth_user SET password =?, edited_By=? WHERE id =?`;
  ...
  bcrypt.genSalt(10, (err, salt) => {
    ...
    bcrypt.hash(password, salt, (err, hash) => {
      ...
      db.query(query, [hash, username, id], (error, result) => {
