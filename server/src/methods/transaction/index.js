...
const get_Running_Sales = async (req, res) =>{
    const query = `
        SELECT account_accountmodel.cusID AS id, account_accountmodel.name, account_accountmodel.code, account_accountmodel.cusID, sales_model.uuid, sales_model.amount, sales_model.amount_paid, sales_model.due_date  FROM account_accountmodel
        JOIN sales_model ON(account_accountmodel.cusID = sales_model.applicant)
        WHERE sales_model.status=? AND sales_model.md_status=? ORDER BY account_accountmodel.name ASC
    `;
...
const get_Todays_Transactions_By_User = (req, res) =>{
    const {username} = req.user
    const query = `
    SELECT transactionmodel.ticket_no, transactionmodel.pay_method, transactionmodel.amount, 
    account_accountmodel.name, transactionmodel.tx_type, transactionmodel.tx_date, account_accountmodel.code

    FROM transactionmodel
    JOIN account_accountmodel
    ON transactionmodel.account_FK = account_accountmodel.code
    WHERE transactionmodel.postBy_FK =?  ORDER BY transactionmodel.updatedAt DESC
    `;
    db.query(query, [username], (error, result) =>{
        if(error) return res.status(404).send(error.message);
        res.status(200).send(result)
...
const TRANSACTION = {
    ...
    get_Todays_Transactions_By_User
}
